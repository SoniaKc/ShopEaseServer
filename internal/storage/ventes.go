...
func GetAllVentesClient(idClient string) (map[string][]map[string]interface{}, error) {
	rows, err := DB.Query(
		"SELECT idTransaction, login_boutique, nom_produit, nom_adresse, nom_paiement, quantite, total, date_vente, statut FROM ventes WHERE idClient = $1 ORDER BY idTransaction, date_vente DESC",
		idClient)
	if err != nil {
